...
public class Monster_Ctrl : MonoBehaviour
{
    ...
    //## 호밍 미사일 변수
    [HideInInspector] public GameObject m_HormingLockOn = null;

    // 모스터를 추적하고 있을때 락온 되어있는지 확인
    ...
    void OnTriggerEnter2D(Collider2D coll)
    {
        ...
        {
            if (coll.gameObject.name.Contains("HomingMissle") == true)
            {
                TakeDamage(150.0f); 
                Destroy(coll.gameObject);
                Game_Mgr.Inst.IncreaseScore(10);
            }
            else
            {
